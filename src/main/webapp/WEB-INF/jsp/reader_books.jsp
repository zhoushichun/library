...
<div style="padding: 20px 550px 10px">
    <form method="post" action="reader_querybook_do.html" class="form-inline" id="searchform">
        <div class="input-group">
            <input type="text" placeholder="输入图书名、类目、简介" class="form-control" id="search" name="searchWord"
                   class="form-control">
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${books}" var="book">
                <tr>
                    ...
                    <c:if test="${flag}">
                        <td><a href="returnbook.html?bookId=<c:out value="${book.bookId}"></c:out>&bookName=<c:out value="${book.name}"></c:out>">
                            <button type="button" class="btn btn-danger btn-xs">归还</button>
                        </a></td>
                    </c:if>
                    <c:if test="${not flag}">
                        <c:if test="${book.number>0}">
                            <td><a href="lendbook.html?bookId=<c:out value="${book.bookId}"></c:out>&bookName=<c:out value="${book.name}"></c:out>">
